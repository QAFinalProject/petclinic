pipeline {
    ...
    stages {
        ...
        stage('Deploy App') {
            steps {
                sh 'ssh -i /home/jenkins/.ssh/aws-key-london.pem ubuntu@18.170.71.60  sudo docker stack deploy --compose-file docker-compose.yaml petclinic-stack'
                }
            }
        stage('Deploy nginx') {
            steps {
                sh 'ssh -i /home/jenkins/.ssh/aws-key-london.pem ubuntu@18.132.192.154 ./nginx-lb-script.sh'
            }
        }
    }
}
